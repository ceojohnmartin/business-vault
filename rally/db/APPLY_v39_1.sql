-- RALLY v39.1 — APPLY 0007 AS ONE TRANSACTION.
--
-- Paste this WHOLE file into the Supabase SQL editor and run it once, AFTER
-- APPLY_v39.sql has been applied and confirmed (STEP 2 of the cutover).
--
-- What it does: replaces scrub_customer_payment() with 0004's body carrying
-- ONE changed rule — last4 is exactly four ASCII digits or the key is absent —
-- and passes every stored customer row through the trigger once, so the row
-- that test/verify-production.sql probe 12 found in production (an empty
-- last4 kept by 0006) is rebuilt without it. The full story is at the top of
...
--   db/migrations/0007_last4_strict.sql
-- (regenerate with db/build-apply.sh if it changes.)
--
-- If it errors: nothing changed. Read the message, fix, run it again.
-- Then verify with test/verify-production.editor.sql — probe 12 must PASS.

begin;

-- ============================ 0007_last4_strict.sql ============================
