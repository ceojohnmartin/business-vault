...
#
# The constraint is DEFERRED, so its check — and the advisory lock the check
# takes — normally runs at COMMIT. A test that simply slept between the two
# inserts would prove nothing: the sleeper reaches COMMIT last and is refused
# by the ordinary check, with no contention at all.
#
# So each session runs SET CONSTRAINTS ALL IMMEDIATE straight after its
# insert. That fires the constraint's check THEN, inside the still-open
# transaction, and the check's own advisory lock is taken and held until
# that transaction ends — exactly the state a real commit-in-flight is in.
# The winner then sleeps with the lock held; the loser arrives a second
# later and its check contends:
#
#   A: BEGIN; insert; SET CONSTRAINTS ALL IMMEDIATE (check runs, lock taken);
#      sleep 4; COMMIT
#   B: starts 1s later; insert; SET CONSTRAINTS ALL IMMEDIATE -> BLOCKS on the
#      team lock until A commits, then sees A's row and refuses.
#
# Case 3 removes the lock from the trigger and runs the IDENTICAL script:
# B's check no longer blocks, sees no committed row, passes, and both commit.
# Without that negative control, case 1 would only prove that a check
# exists. Nothing here depends on two commits landing in the same
# millisecond: the only timing assumption is that B finishes inside A's
# 4-second hold, which is a margin, not a race.
#
...
# SET CONSTRAINTS ALL IMMEDIATE runs the deferred check NOW, inside the open
# transaction, so the check's own team lock is taken and — when $4 is set —
# held across the sleep. That is the constraint's real mechanism, not a
# stand-in for it.
draw() {
...
( draw "$LEAD" ra "0,0,100,100" 4 >/tmp/rally-turf-a.out 2>&1 ) &
sleep 1
START="$(date +%s)"
...
# `! grep -q` — not `grep -qv`, which inverts LINE matching and passes as
# soon as any one line lacks the word, which the set_config echo always does
say "$(! echo "$A_OUT" | grep -qi 'ERROR' && echo 1)" \
...
SQL
# The IDENTICAL interleaving as case 1. A's check ran and passed (no lock to
# hold now); B's check runs while A is still open, sees no committed row,
# passes; both commit. One attempt — there is no race to win.
psql -q -d "$DB" -c "delete from public.territories where team_id='$TEAM'" >/dev/null
( draw "$LEAD"  re "0,0,100,100" 4 >/tmp/rally-turf-e.out 2>&1 ) &
sleep 1
draw "$LEAD2" rf "50,0,150,100" >/tmp/rally-turf-f.out 2>&1 || true
wait
LIVE3="$(psql -d "$DB" -Atc "select count(*) from public.territories where team_id='$TEAM' and deleted_at is null")"
say "$([ "$LIVE3" = "2" ] && echo 1)" \
    "NEGATIVE CONTROL: without the lock BOTH overlapping hoods commit (got $LIVE3) — so case 1 proves the lock"
