-- RALLY v41 — STAGE C. APPLY 0016 (the authoritative overlap invariant)
-- AS ONE TRANSACTION.
--
-- Run ONCE, after db/APPLY_v41_B2.sql has committed, its verification reads
-- 0 FAIL, and db/preflight/v41-preflight-stagec.editor.sql reads
-- GO — Stage C may be applied.
--
-- What it does. The body below is the VERBATIM migration file 0016:
--   * rally_overlap_m2 — the ONE overlap expression. Planar intersection
--     for exact topology; ST_CollectionExtract(...,3) keeps only the
--     polygonal part, so a shared edge (a LINESTRING) and a corner touch (a
--     POINT) both measure exactly 0 and are ALLOWED; the area is taken in
--     geography for true square metres. It FAILS CLOSED: a pair GEOS cannot
--     measure is refused with the reason, never admitted on a guess.
--   * rally_overlap_tolerance_m2 — 1.0 m², a FIXED ABSOLUTE. A relative
--     term would let the permitted absolute overlap grow with the hood,
--     which is the opposite of the rule.
--   * assert_no_turf_overlap — the check, and the team-scoped
--     pg_advisory_xact_lock that makes it safe against two managers drawing
--     at the same instant. Deferral fixes ordering WITHIN a transaction;
--     only the lock stops two concurrent ones each passing against a
--     snapshot that excludes the other.
--   * territories_no_overlap — a DEFERRABLE INITIALLY DEFERRED constraint
--     trigger. Deferral is REQUIRED by Smart Split, not merely preferred:
--     0005 tombstones the parent and inserts the children in one
--     transaction, and mid-transaction the children overlap the still-live
--     parent enormously. Only the FINAL state is judged.
--   * no table, no column, no row rewrite, no grant to any client role
--     beyond what the measurement needs to run inside the check.
--
-- THE ARMING GATE, inside the same transaction. Before the trigger is
-- created, 0016 counts (a) live hoods with an unusable outline — invisible
-- to the index and therefore unprotected — and (b) live pairs that ALREADY
-- overlap. Either one raises and the whole paste rolls back, because arming
-- over an existing collision would leave both hoods of every pair
-- permanently unwritable: the next edit to either, however unrelated, would
-- be refused for a problem that predates the rule. The preflight lists both
-- populations; production reads 0 and 0.
--
-- What it does NOT do: it does NOT touch assignment_server_authoritative,
-- which stays FALSE. No client change, no republish, no other migration.
--
-- Transactional: all or nothing; proven by db/test/stage-c-test.sh.
-- Verify afterwards with db/test/verify-v41-stage-c.editor.sql.
-- Reversible with db/ROLLBACK_v41_C.sql.

begin;

-- ============================ 0016_turf_overlap.sql ============================
