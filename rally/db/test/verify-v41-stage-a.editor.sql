...
begin
  ...
  res := res || pg_temp.a_row('B5 PROOF 5 (after): unresolved historical entries are tagged, never dropped — ' || n || ' tagged', true, 'expected 0 on production (preflight 3b was empty)');
  ...
  select count(*) filter (where deleted_at is null and not archived),
         ...
         count(*) filter (where deleted_at is null and not archived and geom is null
                            and jsonb_typeof(polygon) = 'array' and jsonb_array_length(polygon) > 0)
    into n, n2, n3 from public.territories;
  res := res || pg_temp.a_row('C1 every LIVE hood with an outline has a valid geom', n3 = 0 and n = n2, n || ' live, ' || n2 || ' with valid geom, ' || n3 || ' with an outline but NULL geom');
  ...
  begin
    ...
    select t.id, t.name, t.polygon, t.data, t.assignees, t.assignees_rev
      into hood_id, hood_name, hood_poly, hood_data, hood_led, hood_rev
      from public.territories t
     where t.team_id = team and t.deleted_at is null and not t.archived and t.geom is not null
       and jsonb_typeof(t.data) = 'object' order by t.id limit 1;
    res := res || pg_temp.a_row('D0 SETUP', true, 'team ' || team || ' — probing as a real rep and a real leader' || coalesce(', hood ' || hood_id, ', no live hood on this team (D1/D2 skipped)'));
