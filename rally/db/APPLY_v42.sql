-- RALLY v42 — TERRITORY IDENTITY, SERVER-SIDE PROPERTY IMPORT, SELECTIVE
-- RE-KNOCK. ONE TRANSACTION. Paste the whole file into the Supabase SQL
-- Editor and run it once.
--
-- NOT YET APPLIED TO PRODUCTION. Do not run it until the owner has read the
-- report and approved it.
--
-- WHAT IT CHANGES
--   territories  + seq, + uuid, + cycle_keep        (all server-owned)
--   events       + territory_id, + prev_disposition (a phone may write both)
--   pins         nothing — three EXPRESSION indexes over columns that
--                already exist, so no phone pulls a byte more than today
--   functions    + territories_number, + import_territory_doors,
--                + reset_territory_outcomes, + rally_territory_summary
--   triggers     + territories_number on public.territories
--
-- WHAT IT DOES NOT CHANGE
--   No existing trigger body, policy, grant on an existing column, or RPC
--   is replaced. The assignment ledger is untouched: proven on a local
--   replica of production's current state (0001-0017 + the flip) by
--   db/test/v42-territory-test.sh, which fingerprints every territory, pin
--   and event row before and after and requires them byte-identical.
--
-- BEFORE YOU RUN IT
--   Run db/preflight/v42-import-preflight.editor.sql first. It is read-only
--   and it answers the one question this file cannot answer for itself:
--   whether production already holds duplicate property rows.
--
-- IF IT FAILS
--   It is one transaction, so a failure leaves nothing behind. The
--   in-file guard at the end raises rather than commit if any of the three
--   new server-owned columns turned out to be writable by `authenticated`.
--
-- ROLLBACK: db/ROLLBACK_v42.sql. It drops what this file added. It cannot
-- and does not restore a hood's number once dropped, so re-applying issues
-- new numbers in the same creation order.

begin;

-- ============================ 0018_territory_properties.sql ============================
